          ...
          dangerouslySetInnerHTML={{
            __html: `
              ...
              (function() {
                ...
                // being ragged exactly as it stops moving. Without that taper
                // the filter was switched off in one step at the end, and the
                // notches it had been chewing into the card's own border
                // popped solid in a single frame — the visible "snap" at the
                // end of the fill (measured: 74.2% of the card inked with the
                // filter on vs 76.0% with it off, at an identical radius).
                var disp = document.querySelector('#tantu-rumal-bleed feDisplacementMap');
                var BASE_SCALE = disp ? (parseFloat(disp.getAttribute('scale')) || 0) : 0;
                // The soak blur is tapered alongside the displacement. Zeroing
                // only the displacement still left this softening the card's
                // border by ~2px, so switching the filter off at the end was
                // still a (smaller) visible step. With both at 0 the filter is
                // an identity pass by the final frame, and removing it changes
                // nothing on screen.
