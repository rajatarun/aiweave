 ...
 * measured yet.
 */
export function inlineFlip(el: Element | null | undefined): 1 | -1 {
  if (!el || typeof window === "undefined" || !window.getComputedStyle) return 1;
  try {
 ...
 */
export function inlineArrowStep(key: string, el: Element | null | undefined): -1 | 0 | 1 {
  if (key !== "ArrowRight" && key !== "ArrowLeft") return 0;
  const forward = key === "ArrowRight" ? 1 : -1;
  return (forward * inlineFlip(el)) as -1 | 1;
}
 ...
 */
export function inlineStartPadding(style: CSSStyleDeclaration): number {
