...
export interface MakuShuttleHandle {
  dispose(): void;
}
...
export function createMakuShuttle(
  svg: SVGSVGElement,
  /**
   ...
   */
  coordEl: HTMLElement | null,
  options: MakuShuttleOptions = {},
): MakuShuttleHandle {
  const { throwDuration = 180, trailDuration = 520, tensionWindow = 260, spatialRouting = true } = options;

  let lastPoint: { x: number; y: number } | null = null;
  let lastTime = 0;
  let tautPoints: Array<{ x: number; y: number }> = [];
  let focused: HTMLElement | null = null;

  function drawThread(points: Array<{ x: number; y: number }>, taut: boolean) {
  ...
  function handleFocusOut() {
    window.setTimeout(() => {
      if (document.activeElement === document.body) {
        ...
        focused?.removeAttribute("data-maku-coord");
        focused = null;
        if (coordEl) coordEl.setAttribute("data-state", "hidden");
      }
    }, 0);
  }
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    document.querySelectorAll<HTMLElement>(INTERACTIVE_SELECTOR).forEach((node) => {
      if (node === current || !isReachable(node)) return;
      const rect = node.getBoundingClientRect();
