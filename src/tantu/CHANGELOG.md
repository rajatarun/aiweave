...
All notable changes to `@weaveaijs/tantu`.
...
- **Fonts are decoupled from the design system.** The stylesheet named three
  ...
  to stacks every machine already has. `@weaveaijs/tantu/styles.css` is a
...
- **`@weaveaijs/tantu/fonts.css`** — an optional import that ships the three
